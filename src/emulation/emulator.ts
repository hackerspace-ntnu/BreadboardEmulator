...
type RegisterFile = Array<Register>;
...
function verify16BitNum(value: number): number { return value < 0 ? 65535 + value : value > 65535 ? value - 65536 : value; }

export function CPUStep(state: ICPUState) {
    // Fetch
    const fetchedInstruction = ('0000000000000000' + state.memory[state.PC]).toString().slice(-16);
